...
import {
  Clock, CalendarDays, X, ArrowLeft, Loader2, UserCog, CalendarClock,
} from 'lucide-react';
...
export default function PainelColaborador() {
  ...
  const load = async () => {
    ...
    try {
      const [appts, sched, pros] = await Promise.all([
        base44.entities.Appointment.filter({ professional_name: proName }, 'date', 50),
        base44.entities.Schedule.filter({ professional_name: proName }),
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      ...
      if (isNewSchedule) {
        ...
        await base44.entities.Schedule.update(schedule.id, payload);
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleMoveClient = async () => {
    ...
    try {
      await base44.entities.Appointment.update(moveAppt.id, { professional_name: moveTo });
      setMoveAppt(null);
      setMoveTo('');
      load();
    } catch (e) {
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section>
              <h2 className="text-xs uppercase tracking-widest text-[#2b2622]/50 font-medium mb-4 flex items-center gap-2">
                <CalendarClock size={15} className="text-[#B67D35]" /> Próximos ({upcomingAppts.length})
              </h2>
              <div className="space-y-3">
          ...
          <aside>
            <div className="bg-white border border-[#E0DCD6] rounded-2xl p-5 lg:sticky lg:top-24">
              <h2 className="text-xs uppercase tracking-widest text-[#2b2622]/50 font-medium mb-4 flex items-center gap-2">
                <UserCog size={15} className="text-[#B67D35]" /> Meus Horários
              </h2>
              {isNewSchedule && (
                <p className="text-[11px] text-[#B67D35] bg-[#B67D35]/5 rounded-lg px-3 py-2 mb-3">
                  Configure sua grade de horários. Os clientes só verão estes horários disponíveis.
                </p>
              )}
              <div className="space-y-3">
                {WEEKDAYS.map((d) => (
                ...
                >
                  {saving ? <Loader2 className="animate-spin mx-auto" size={16} /> : 'Salvar Horários'}
                </button>
              </div>
            </div>
          </aside>
