...
export async function GET(request: NextRequest) {
  try {
    ...
    const appointments = agendamentos.map((a) => ({
      ...
      data: dateToString(a.data),
      horaInicio: timeToString(a.horaInicio),
      horaFim: timeToString(a.horaFim),
      valorTotal: a.valorTotal,
      status: STATUS_REVERSE[a.status] || a.status,
      formaPagamento: PAGAMENTO_REVERSE[a.formaPagamento] || a.formaPagamento,
      criadoEm: a.criadoEm,
      cliente: a.cliente,
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ appointment: { ...agendamento, data: dateToString(agendamento.data), horaInicio: timeToString(agendamento.horaInicio), horaFim: timeToString(agendamento.horaFim), status: STATUS_REVERSE[agendamento.status] || agendamento.status, formaPagamento: PAGAMENTO_REVERSE[agendamento.formaPagamento] || agendamento.formaPagamento, profissional: mapProfissional(agendamento.profissional) } }, { status: 201 });
  } catch (error) {
