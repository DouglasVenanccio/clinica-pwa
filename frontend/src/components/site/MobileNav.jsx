...
import {
  ...
  UserCog, LogOut, LayoutDashboard,
} from 'lucide-react';
        ...
        <div className="lg:hidden fixed inset-0 z-50" onClick={() => setSheetOpen(false)}>
          ...
          >
            ...
            <div className="space-y-1">
              {isAuthenticated ? (
                <>
                  <SheetLink to="/meus-agendamentos" icon={Calendar} label="Meus Agendamentos" onClick={() => setSheetOpen(false)} />
                  ...
                  {(user?.role === 'ADMIN' || user?.role === 'PROFISSIONAL') && (
                    <SheetLink to="/dashboard" icon={LayoutDashboard} label="Painel" onClick={() => setSheetOpen(false)} />
                  )}
                  <button
